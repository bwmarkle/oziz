 ...
 * @since 2.0.0
 */
class Oziz_Category_Control extends WP_Customize_Control
{
...
if (! empty($this->description) ) :
    ?><span class="description customize-control-description"><?php echo wp_kses_post($this->description); ?></span><?php
        ...
        $dropdown_args = wp_parse_args(
            $this->dropdown_args, array(
            'taxonomy'          => 'category',
            'show_option_none'  => '',
            'selected'          => $this->value(),
            'show_option_all'   => __('All', 'oziz'),
            'orderby'           => 'id',
            'order'             => 'ASC',
            'show_count'        => 1,
            'hide_empty'        => 1,
        ...
        $dropdown = str_replace('<select',  $this->get_link(), $dropdown);
