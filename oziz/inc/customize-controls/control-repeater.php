 ...
 * @access public
 */
class Oziz_Customize_Repeatable_Control extends WP_Customize_Control
{
    ...
    public function __construct( $manager, $id, $args = array() )
    {
        ...
        if (! isset($args['allow_unlimited']) || $args['allow_unlimited'] != false ) {
            $this->max_item =  apply_filters('oziz_reepeatable_max_item', absint($args['max_item']));
        }  else {
            $this->max_item = absint($args['max_item']);
        }

        $this->changeable =  isset($args['changeable']) && $args['changeable'] == 'no' ? 'no' : 'yes';
        $this->default_empty_title =  isset($args['default_empty_title']) && $args['default_empty_title'] != '' ? $args['default_empty_title'] : esc_html__('Item', 'oziz');
    ...
    public function render_content()
    {
        ...
        <div class="repeatable-actions">
            <span class="button-secondary add-new-repeat-item"><?php echo esc_html($this->add_text) ? esc_html($this->add_text) : esc_html_e('Add an item', 'oziz'); ?></span>
        </div>
        <?php
    }
            ...
            <div class="widget">
                <div class="widget-top">
                    ...
                    <div class="widget-title">
                        <h4 class="live-title"><?php esc_html_e('Item', 'oziz'); ?></h4>
                    </div>
                </div>
                            ...
                            <div data-field-id="{{ field.id }}" class="field--item conditionize item item-{{ field.type }} item-{{ field.id }}" data-cond="{{ JSON.stringify( field.required ) }}" >
                                ...
                                <div data-field-id="{{ field.id }}"  class="field--item item item-{{ field.type }} item-{{ field.id }}" >
                                    ...
                                    <select data-live-id="{{ field.id }}"  class="select-multiple" multiple="multiple" data-repeat-name="_items[__i__][{{ field.id }}][]">
                                        ...
                                        <div class="actions">
                                            <button class="button remove-button " <# if ( ! field.value.url ){ #> style="display:none"; <# } #> type="button"><?php esc_html_e('Remove', 'oziz') ?></button>
                                            <button class="button upload-button" data-media="{{field.media}}" data-add-txt="<?php esc_attr_e('Add', 'oziz'); ?>" data-change-txt="<?php esc_attr_e('Change', 'oziz'); ?>" type="button"><# if ( ! field.value.url  ){ #> <?php esc_html_e('Add', 'oziz'); ?> <# } else { #> <?php esc_html_e('Change', 'oziz'); ?> <# } #> </button>
                                        ...
                                        <a href="#" class="remove-icon"><?php esc_html_e('Remove', 'oziz'); ?></a>
                                        <# }  #>
                                ...
                                <div class="widget-control-actions">
                                    ...
                                    <span class="remove-btn-wrapper">
                                        <a href="#" class="repeat-control-remove" title=""><?php esc_html_e('Remove', 'oziz'); ?></a> |
                                    </span>
                                        <a href="#" class="repeat-control-close"><?php esc_html_e('Close', 'oziz'); ?></a>
                                    </div>
                                    <br class="clear">
                                </div>
